HTMLWidgets.widget({
  ...
  renderValue: function(el, x, instance) {
    ...
    //try {
      ...
      /*  for constant layout such as igraph layout
      ...
      */
      ...
      if( !(x.network === null) ){
        ...
        x.network.nodes_df = HTMLWidgets.dataframeToD3(x.network.nodes_df);
        x.network.edges_df = HTMLWidgets.dataframeToD3(x.network.edges_df);
        
        
        // don't use Array map so will work in RStudio
        var nodecount = x.network.nodes_df.length, i = 0;
        for( ; i < nodecount; i ++  ){
          graph.addNode(
            x.network.nodes_df[i].nodes,
            x.network.nodes_df[i]
          )
        }
        
        // don't use Array map so will work in RStudio
        var edgecount = x.network.edges_df.length, i = 0;
        for( ; i < edgecount; i ++  ){
          graph.addLink(
            x.network.edges_df[i].from,
            x.network.edges_df[i].to,
            x.network.edges_df[i]
          )
        }
      }
